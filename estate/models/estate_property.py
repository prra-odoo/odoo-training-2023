...
class estate_property(models.Model):
    ...
    date_availability = fields.Date(
        string="Available From", default=fields.Date.today()+relativedelta(months=3), copy=False)
    ...
    tag_ids = fields.Many2many(
        "estate.property.tag", "property_tag_rel", "property_id", "tag_id", string="Tags")
    ...
    garden_area = fields.Integer(
        string="Garden Area (sqm)", compute='_compute_garden_area', store=True, readonly=False)
        ...
        for record in self:
            if record.selling_price < record.expected_price * .9:
                raise exceptions.ValidationError(
                    r"the selling price cannot be lower than 90% of expected price ")
            else:
                return True
